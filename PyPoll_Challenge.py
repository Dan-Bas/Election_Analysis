...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in counties_votes:
        ...
        votes_counties = counties_votes[county_name]

        # 6c: Calculate the percent of total votes for the county.
        percentage_counties = float(votes_counties) / float(total_votes) * 100
        
         # 6d: Print the county results to the terminal.
        counties_results = (
            f"\n{county_name}: {percentage_counties:.1f}% ({votes_counties:,})\n")
        ...
        if (votes_counties > largest_count):
            largest_count = votes_counties
